...
interface Props {
  ...
  // Where the camera opens when there's no live GPS fix — the active venue's centre.
  center: Coordinates
  defaultZoom?: number
...
/**
 * 3D map view — the same contract as FloorPlanMap, rendered with MapLibre GL at
 * a 60° tilt with extruded buildings. The shared leafletMapRef handle is adapted
 * so the page's flyTo / re-centre logic works unchanged in either view, and it's
 * driven entirely by the active venue (centre, floor plans, waypoints) rather
 * than any hard-coded place.
 */
export default function Map3DView({
  ...
  useEffect(() => {
    ...
    const start = positionRef.current ?? center
  ...
  }, [currentFloor, floorPlans, styleReady])
