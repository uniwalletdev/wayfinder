...
import { BarList, COVERAGE_SHADES, ColumnChart, ShareBar } from "@/components/admin/charts"
import {
  Badge,
  Card,
  EmptyState,
...
import { catalogSummary } from "@/lib/admin/catalog"
import { getOverview } from "@/lib/admin/data"
import type { Overview } from "@/lib/admin/types"
import { statusBadge } from "@/components/admin/labels"
    ...
    <>
      ...
      <section className="mb-6 grid gap-5 lg:grid-cols-[minmax(0,340px)_minmax(0,1fr)]">
        ...
        <Card
          ...
          actions={
            <Link href="/admin/catalog" className="text-[12.5px] font-semibold text-wf-primary">
              Browse →
            </Link>
          }
        >
        ...
        >
          {overview.reason === "unconfigured" ? (
            <p>
              ...
              activity — then this page fills in. <Link href="/admin/settings" className="underline">Check the settings</Link>.
            </p>
          ) : (
    ...
    <>
      <section className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatTile
          ...
          href="/admin/venues?sort=waypoints"
        />
        <StatTile
      ...
      <section className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,380px)]">
        <Card
          ...
          actions={
            <Link href="/admin/venues" className="text-[12.5px] font-semibold text-wf-primary">
              All venues →
            </Link>
          }
          bodyClassName=""
                ...
                <tbody>
                  {data.recentVenues.map((v) => {
                    ...
                    return (
                      <tr key={v.id}>
                        <td className={TD}>
                          <Link href={`/admin/venues/${v.id}`} className="font-semibold text-wf-ink hover:text-wf-primary">
                            {v.name}
                          </Link>
        ...
        <div className="flex flex-col gap-5">
          <Card
            ...
            actions={
              <Link href="/admin/demand" className="text-[12.5px] font-semibold text-wf-primary">
                All →
              </Link>
            }
          >
          ...
          <Card
            ...
            actions={
              <Link href="/admin/audit" className="text-[12.5px] font-semibold text-wf-primary">
                Full log →
              </Link>
            }
          >
