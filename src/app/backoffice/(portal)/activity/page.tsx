...
import { Badge, Card, EmptyState, Notice, PageHeader, StatTile, TABLE, TABLE_SCROLL, TD, TH, formatDateTime, formatNumber, timeAgo } from "@/components/backoffice/ui"
import { getActivity, isUuid } from "@/lib/backoffice/data"
                        ...
                        <tr key={v.venueKey} className="transition-colors hover:bg-wf-surface-2">
                          <td className={TD}>
                            {v.source === "shared" && isUuid(v.venueKey) ? (
                              <Link href={`/backoffice/venues/${v.venueKey}`} className="font-medium text-wf-ink hover:text-wf-primary">
                                {v.venueLabel}
                              </Link>
                            ) : (
